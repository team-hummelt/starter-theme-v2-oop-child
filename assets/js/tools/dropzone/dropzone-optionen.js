                    ...
                    switch (response.handle) {
                        case'dokument_upload':
                            ...
                            html.innerHTML = `<td id="current${response.id}" class="align-middle text-center"><span class="table-file file ext_${response.ext}"></span></td>
                               ...
                               <td class="align-middle text-start"><span class="lh-1 mb-0">${response.date}<small class="small-lg d-block">${response.time} Uhr</small> </span></td>
                               <td class="align-middle text-center">${response.size}</td>
                               <td class="align-middle text-start">${response.type}</td> 
                               <td class="align-middle text-center">
                               <button data-id="${response.id}" data-type="delete_document" data-handle="upload_table" class="child-action btn btn-outline-danger text-nowrap btn-sm disabled" disabled><i class="bi bi-trash"></i>&nbsp; löschen</button>
                               </td>`;
